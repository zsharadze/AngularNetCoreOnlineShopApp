...
namespace ASPNetCoreWebApi.Repositories
{
    public class OrderRepository : IOrderRepository, IAsyncDisposable
    {
        ...
        private Expression<Func<Order, OrderDTO>> OrderListExpression = x =>
              new OrderDTO
              {
                  Id = x.Id,
        ...
        public async Task<int> ChangeStatus(ChangeOrderStatusDTO changeOrderStatusRequest)
        {
            var order = await _context.Orders.FindAsync(changeOrderStatusRequest.OrderId);
            if (order == null)
            {
                throw new Exception($"Order with id: {changeOrderStatusRequest.OrderId} not found.");
            }

            order.Status = changeOrderStatusRequest.NewStatus;
