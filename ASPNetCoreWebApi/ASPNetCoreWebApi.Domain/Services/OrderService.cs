...
namespace ASPNetCoreWebApi.Domain.Services
{
    public class OrderService : IOrderService
    {
        ...
        public async Task<int> CreateOrder(List<CreateOrderData> orderItemList, string promoCode, string userId)
        {
            decimal subTotal = 0;
            var productIds = orderItemList.Select(x => x.ProductId).ToList();
            var orderItemsProducts = await _productService.GetByIds(productIds);
        ...
        public Task<int> ChangeStatus(ChangeOrderStatusDTO changeOrderStatusRequest)
        {
            return _repository.ChangeStatus(changeOrderStatusRequest);
        }
    }
}
